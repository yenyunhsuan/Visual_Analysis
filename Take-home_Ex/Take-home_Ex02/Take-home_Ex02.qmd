...
# Egocentric of high degree

```{r}
g <- graph_from_data_frame(d=MC2_edges, vertices=MC2_nodes, directed=TRUE)
```

Betweenness centrality is deﬁned as "[the number of shortest paths between nodes that pass through a particular node](https://jtr13.github.io/cc21fall2/network-visualization-in-r.html)".

```{r}
betweenness_centrality <- betweenness(g)
MC2_nodes$betweenness_centrality <- betweenness_centrality[as.character(MC2_nodes$id)]
...
# create data frame of top 3 firms with highest out-degree 
top_in <-
...
MC2_graph <- tbl_graph(nodes = node,
                       edges = top_edge,
                       directed = TRUE)
```
...
```{r}
g <- graph_from_data_frame(d=top_edge, vertices=node, directed=TRUE)
```

```{r}
betweenness_centrality <- betweenness(g)
node$betweenness_centrality <- betweenness_centrality[as.character(node$id)]
head(sort(betweenness_centrality, decreasing=TRUE))
```
